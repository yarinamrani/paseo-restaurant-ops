                ...
                <div key={v.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      <h4 className="font-semibold text-slate-900">{v.profession}</h4>
                      {v.name ? (
    ...
    <Modal title={vendor ? `עריכה — ${vendor.profession}` : 'מקצוע חדש'} onClose={onClose}>
      <form onSubmit={save} className="space-y-3">
        ...
        <label className="block text-sm">
          <span className="mb-1 block text-slate-600">קטגוריה</span>
        ...
        />
        <label className="block text-sm">
          <span className="mb-1 block text-slate-600">דירוג</span>
